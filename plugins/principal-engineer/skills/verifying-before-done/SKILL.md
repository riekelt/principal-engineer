...
2. **Run the verify command, paste its output.** The verify command comes from the pre-change checkpoint; it proves the gates hold, and its actual output backs that part of the claim. "Verified" is always "PASS (checked X and Y)", never a bare checkmark.
...
- **Quote what the system produced.** The response body, the terminal output, and the rendered screen back the claim. Treat ambiguous output as a failure with the raw capture attached, never interpret it into a pass.
